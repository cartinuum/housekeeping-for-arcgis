...
interface DashboardProps {
  ...
  orgDataSource?: OrgDataSource  // which path was used — drives context text
}
...
export function Dashboard({ items, orgTotal, orgDataSource }: DashboardProps) {
      ...
      {viewScope === 'org' && activeView === 'treemap' && (
        <div style={{ padding: '0 8px 4px', fontSize: 12, color: 'var(--calcite-color-text-3)' }}>
          {orgDataSource === 'fallback-storage'
            ? <>Top {treemapItems.length} items by storage across sampled users</>
            : <>Top {treemapItems.length} of {items.length} items by credit consumption (last 3 months)</>
          }
          {orgTotal !== undefined && <>. Organisation total: {orgTotal >= 10_000 ? '10,000+' : orgTotal.toLocaleString()} items</>}.
          {treemapClipped && <> Switch to Table for the full {filtered.length.toLocaleString()} item list.</>}
        </div>
      )}

      {/* Storage metric in org/analytics mode — hosted services have size:-1 so only
          file-backed items (Service Definitions, CSVs etc.) appear in the treemap. */}
      {viewScope === 'org' && orgDataSource === 'analytics' && activeView === 'treemap' && effectiveMetric === 'size' && (
        <div style={{ padding: '0 8px 2px', fontSize: 11, color: 'var(--calcite-color-text-3)' }}>
          Storage sizes are unavailable for hosted services in org mode. Switch to Credits to see all {items.length} items.
        </div>
      )}
