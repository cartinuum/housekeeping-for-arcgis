...
const DEFAULT_FILTERS: Filters = {
  types: [],
  minSizeBytes: 5_242_880,   // 5 MB default for own scope — keeps noise out of large personal inventories
  maxSizeBytes: null,
...
// Org scope: analytics endpoint pre-ranks by credit cost — size filter is wrong here.
// A notebook with 0 bytes but high query volume costs real credits. Start with no size minimum.
const ORG_DEFAULT_FILTERS: Filters = {
  ...DEFAULT_FILTERS,
  minSizeBytes: null,
}
...
export const useAppStore = create<AppStore>((set) => ({
  ...
  setViewScope: (scope) => set({ viewScope: scope, selectedIds: [], filters: scope === 'org' ? ORG_DEFAULT_FILTERS : DEFAULT_FILTERS }),
